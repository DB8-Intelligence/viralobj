...
const VOICE_PROFILES = {
  ...
  // Professional / Authoritative (Type G)
  professional: {
    voice_id: "Friendly_Person",
    emotion: "neutral",
    speed: 1.0,
...
export async function generateVideo({
  ...
  for (const char of pkg.characters) {
    ...
    // —— STEP 1: Generate character image via FLUX Pro v1.1 ——————————————————
  ...
  const result = {
    meta: {
      ...
      pipeline: "FLUX Pro v1.1 → MiniMax TTS → VEED Fabric",
    },
...
function estimateCost(clips, quality) {
  ...
  const costPerImage = quality === "premium" ? 0.045 : 0.03;  // FLUX Pro v1.1 per 1MP (9:16 → 1.5MP)
